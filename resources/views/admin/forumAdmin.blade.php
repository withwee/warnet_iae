...
<section class="space-y-4">
    ...
    <div class="bg-white p-4 rounded-xl shadow">
        <form action="{{ route('admin.forum.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex items-center gap-3 mb-3">
                <img src="{{ auth()->user()->photo ? asset('storage/' . auth()->user()->photo) . '?v=' . time() : asset('images/Foto Profil.jpg') }}" 
                     alt="Profile Photo" 
    ...
    <div class="bg-white p-4 rounded-xl shadow">
        <div class="flex items-center justify-between mb-3">
            ...
            {{-- Menu dropdown - Admin dapat menghapus semua postingan --}}
            <div class="relative">
                ...
                <div id="dropdown-post-{{ $forum->id }}" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    <form id="deletePostForm-{{ $forum->id }}" action="{{ route('admin.forum.deletePost', $forum->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
        ...
        <div class="border-t border-gray-200 pt-3">
            ...
            <div class="flex gap-3 mb-2 items-start">
                ...
                {{-- Menu dropdown - Admin dapat menghapus semua komentar --}}
                <div class="relative">
                    ...
                    <div id="dropdown-comment-{{ $comment->id }}" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                        <form id="deleteCommentForm-{{ $comment->id }}" action="{{ route('admin.forum.deleteComment', $comment->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
