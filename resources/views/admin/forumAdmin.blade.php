...
<section class="space-y-4 p-4" x-data="{ openPostMenu: null, openCommentMenu: null }">
    ...
    <div class="bg-white p-4 rounded-xl shadow">
        <div class="flex items-center justify-between mb-3">
            ...
            <div class="relative">
                <button @click="openPostMenu = openPostMenu === {{ $forum->id }} ? null : {{ $forum->id }}" class="p-2 rounded-full hover:bg-gray-200" title="Opsi Postingan">
                    <iconify-icon icon="mdi:dots-vertical" class="text-xl"></iconify-icon>
                </button>
                <div x-show="openPostMenu === {{ $forum->id }}" @click.away="openPostMenu = null" class="absolute right-0 mt-2 w-32 bg-white border rounded shadow-lg z-10">
                    <form action="{{ route('admin.forum.deletePost', $forum->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus postingan ini?');" class="p-2">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full text-left text-red-600 hover:text-red-800 font-bold flex items-center gap-1">
                            <iconify-icon icon="mdi:delete-outline" class="text-lg"></iconify-icon> Hapus
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div class="border-t border-gray-200 pt-3">
            <!-- Form komentar -->
            <form action="{{ route('admin.forum.reply', $forum->id) }}" method="POST" class="flex gap-2 mb-3">
                @csrf
                <input type="text" 
                       name="komentar" 
                       placeholder="Tulis komentar..." 
                       class="flex-1 rounded-full px-4 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                <button type="submit" class="text-blue-500 hover:text-blue-700">
                    <iconify-icon icon="mdi:send" class="text-2xl"></iconify-icon>
                </button>
            </form>
            ...
            <div class="flex gap-3 mb-2">
                ...
                <div class="flex-1">
                    <div class="bg-gray-100 p-2 rounded-lg flex justify-between items-center">
                        ...
                        <div class="relative">
                            <button @click="openCommentMenu = openCommentMenu === {{ $comment->id }} ? null : {{ $comment->id }}" class="p-1 rounded-full hover:bg-gray-200" title="Opsi Komentar">
                                <iconify-icon icon="mdi:dots-vertical" class="text-lg"></iconify-icon>
                            </button>
                            <div x-show="openCommentMenu === {{ $comment->id }}" @click.away="openCommentMenu = null" class="absolute right-0 mt-2 w-32 bg-white border rounded shadow-lg z-10">
                                <form action="{{ route('admin.forum.deleteComment', $comment->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus komentar ini?');" class="p-2">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full text-left text-red-600 hover:text-red-800 font-bold flex items-center gap-1">
                                        <iconify-icon icon="mdi:delete-outline" class="text-base"></iconify-icon> Hapus
                                    </button>
                                </form>
                            </div>
                        </div>
